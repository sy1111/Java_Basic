...
public class CodingEx01 {
	 ...
	 * 입력: 정수 출력: 정수 (총합)
	 * 
	 * 입력받은 정수 미만의 3과 5의 배수의 총합을 구하고, 그 총합을 리턴
	 * 
	 * limit 미만의 자연수만큼 반복 숫자가 3의 배수인지 5의 배수인지 판단 참 --> 더하고
	 * 
	 * 
	 */
	 ...
	 * 입력: 문자열 출력: 변경된 문자열
	 * 
	 * 입력된 문자열에서 탭 캐릭터를 공백4개로 교체 "abc\tacd" --> "abc acd"
	 * 
	 * 1st 해당 기능을 하는 String class 기능
	 * 
	 * 2nd 캐릭터배열 해당문자열의 길이 탭키의 수
	 * 
	 */
	 ...
	 * 게시물의 페이지 수 계산
	 * 
	 * 입력: 총게시물의 수 , 페이지 당 게시물의 수 출력: 페이지 수
	 * 
	...
	/**
	 * 입력: 무작위 정수의 배열 출력: 특별히 소트된 배열
	 * 
	 * 입력값 중 음수가 좌측으로 양수는 우측으로 원래 음수와 양수의 순서는 그대로 유지
	 * 
	 * 로직. 0. 입력 배열의 모든 요소를 돌면서, 음수의 갯수를 센다. 1. 음수 / 양수 배열을 만든다. 2. 입력 배열을 모두 돌면서,
	 * 음의 수가 나오면 음의 배열로, 양의 수가 나오면 양의 배열로 복사 3. 결과 배열을 만든다(입력배열의 크기) 4. 음의 배열의 요소모두를
	 * 결과배열에 복사 5. 양의 배열의 요소 모두를 결과 배열에 복사
	 * 
	 */
